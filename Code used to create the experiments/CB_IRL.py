    ...
    def get_train_data(self):
        ...
        if len(self.agent_data) >= 2:
            ...
            while c < self.hyps["agent_expert_divergence"]:
                ...
                x = np.concatenate((self.agent_data[t_id][o1], self.expert_data[o2]))
                y = np.array([0])
                train_data.append([x,y])
                c += 1
        ...
        c = 0
        while c < 150:
            o1 = np.random.choice(len(self.expert_data) - (self.hyps["frame_size"] // 10))
            o2 = o1 + np.random.choice(self.hyps["frame_size"] // 10)
            x = np.concatenate((self.expert_data[o1], self.expert_data[o2]))
            y = np.array([1])
            train_data.append([x,y])
            c += 1
        ...
        c = 0
        while c < 150:
            ids = np.random.choice(len(self.expert_data), 2)
            if np.abs(ids[0] - ids[1]) < (self.hyps["frame_size"] // 10):
                continue
            x = np.concatenate((self.expert_data[ids[0]], self.expert_data[ids[1]]))
            y = np.array([0])
            train_data.append([x,y])
            c += 1
...
class Embedding():
    ...
    def __init__(self, env, domain, expert_data, hyps):
        self.domain = domain
        self.expert_data = expert_data[0][0]
        self.hyps = hyps

        self.replay_buffer = ReplayBuffer(self.expert_data, hyps)
        self.cbr_obs = []
        self.model = Embedding_Net(env, self.hyps["hidden_size"])
        for i in range(len(self.expert_data)):
            if i % 1 == 0:
                self.cbr_obs.append(self.expert_data[i])
    ...
    def reward(self, obs):
        inputs = []
        for expert_obs in self.cbr_obs:
        ...
        reward = best_expert_obs_id - len(self.cbr_obs)
        reward /= len(self.cbr_obs)
        return reward
...
class newEnvironment(gym.Env):
    ...
    def __init__(self, env, embedding, model_nr, train, reward_style=0):
        self.env = env
    ...
    def step(self, action):
        ...
        if self.reward_style == 0:
            reward = self.t_rews[-1]
        elif self.reward_style == 1:
            reward = 0 if len(self.t_rews) == 1 else self.t_rews[-1] - self.t_rews[-2]
...
def test(nummer, model_nr, domain):
    ...
    newEnv = newEnvironment(env, embedding, model_nr, train=False)
    model = utils.load_model(domain, "ours", model_nr, nummer)
    rews = utils.run_episode(newEnv, model, print_reward=True, render=True, domain=domain)
    print("sum_rewards: ", np.sum(rews))

    print("sum_rewards: ", np.sum(Expert.test_expert(newEnv, domain)))

    print(embedding.cbr_obs[-1])
    exit()
...
test("200000", "9", "half_cheetah_discrete")
...
def train_ours(env, domain, model_nr, load=False, train=True, expert_data=None):
    ...
    newEnv = newEnvironment(env, embedding, model_nr, train=train, reward_style=embedding.hyps["reward_style"])
